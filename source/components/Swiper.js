...
export default class Swiper extends React.Component {
      ...
      onPanResponderRelease: (e, gesture) => {
        ...
        if (
          ...
          return Animated.spring(this.state.pan, { toValue: { x: 0, y: 0 } }).start(() => {
            if (this.props.onAnimationEnd) this.props.onAnimationEnd(this.state.activeIndex);
          });
  ...
  _changeIndex(delta = 1) {
    ...
    if (this.props.direction === 'row') move.x = this.state.width * -1 * calcDelta;
    else move.y = this.state.height * -1 * calcDelta;
    Animated.spring(this.state.pan, { toValue: move }).start(() => {
      if (this.props.onAnimationEnd) this.props.onAnimationEnd(index);
    });
    this.startAutoplay();
          ...
          >
            ...
            <View style={[{ flexDirection: direction }, styles.dotsWrapperStyle, dotsWrapperStyle]}>
              {children.map((el, i) => (
                <View key={i}>
                  {i === activeIndex
                    ? activeDotElement || <View style={[styles.activeDotStyle, activeDotStyle, Styles.ViewStyle.shadowStyle]} />
                    : dotElement || (
                      <View style={[styles.dotStyle, dotStyle, Styles.ViewStyle.shadowStyle]} />
                    )}
                </View>
              ))}
            </View>
            <View
              style={{ opacity: activeIndex + 1 >= this.count ? overRangeButtonsOpacity : 1 }}
            />
          </View>
        </View>
      </View>
