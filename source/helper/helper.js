...
export const numberToCurrency = (number, haveCurrency) => {
  if (!number) {
    return 0;
  }
  if (haveCurrency) {
...
export const currencyToNumber = text => {
  console.log('STRING TO NUMBER', Number(_.replace(text, /,/g, '')), text);
  return Number(_.replace(text, /,/g, ''));
...
export const removeDiacritics = str => {
  ...
  return str.replace(/[^A-Za-z0-9\s]+/g, a => {
    return diacriticsMap[a] || a;
  });
};
export async function changeMode(mode) {
  await Storage.set(Consts.STORAGE.USER_MODE, mode);
}
...
export async function resizeImage(image) {
  console.log('ressize RESpoisse', image);
  const resizeResponse = await ImageResizer.createResizedImage(image.path, 500, 500, 'JPEG', 80, 0);
  console.log('ressize RESasdasdsadsaoisse', resizeResponse);
  const base64Data = await fs.readFile(resizeResponse.path, 'base64');
  return {
    uri: resizeResponse.path,
    base64: base64Data
  };
}
